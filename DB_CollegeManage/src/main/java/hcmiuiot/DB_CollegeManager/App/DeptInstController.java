...
public class DeptInstController implements Initializable {
	...
	private void loadCombo(ActionEvent event) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				ResultSet rsDept = DbHandler.getInstance()
				...
				try {
					...
					while (rsDept.next()) {
						...
						ResultSet rsInstructors = DbHandler.getInstance()
								.execQuery("SELECT * FROM topicS.Instructor WHERE deptID='" + deptID + "'");
						...
						while (rsInstructors.next()) {
							...
							Instructor ins = new Instructor(insID, insName, insBirthday, degreeID, degreeStr, insMail,
									insPhone, img, accessionDate, insdeptID);
							instructors.add(ins);
	...
	class Department {
		...
		public Department(String deptID, String mail, String phone, Image logo, String headID, Instructor head,
		...
		public void getInstructorList(ArrayList<Instructor> instructorList) {
