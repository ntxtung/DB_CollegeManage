package hcmiuiot.DB_CollegeManager.Extensions;
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 0; i < colsCount; i++) {
					if (columnType[i] == Types.VARCHAR || columnType[i] == Types.CHAR)
					...
					else if (columnType[i] == Types.DATE) {
					...
					else if (columnType[i] == Types.BLOB || columnType[i] == Types.BINARY || columnType[i] == -4) {
						...
						currentRow.createCell(i).setCellValue(rs.getLong(i + 1));
				}
			}
